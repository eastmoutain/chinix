...
static char *longlong_to_hexstring(char *buf, unsigned long long u, size_t len, unsigned int flag)
{
    size_t pos = len;
...
#define OUTPUT_STRING(str, len) do { err = print(str, len); if (err < 0) { goto exit; } else { chars_written += err; } } while(0)
